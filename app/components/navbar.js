...
const Navbar = (props) => {
  ...
  useEffect(() => {
    if (
      window.localStorage.getItem("scroll") &&
      window.localStorage.getItem("scroll") !== "null"
    ) {
  ...
  useEffect(() => {
    ...
    if (props.styling.waypoint.setup && window.screen.width > 770) {
      const offset = props.styling.waypoint.offset
        ? props.styling.waypoint.offset
        : 0;
    ...
    for (let link of links) {
      link.addEventListener("click", () => {
        ...
        if (
          ...
          window.location.pathname !== link.getAttribute("data-path")
        ) {
  ...
  return (
    <div style={{ zIndex: 9999, position: "fixed" }}>
      <LanguageSelector
        content={props.content.languageSelector}
        close={toggleLanguageSelector}
      />
      <nav
        id="navbar"
        className={`${classes.navbar} navbar navbar-expand-md navbar-dark mb-4 py-1`}
      >
        <button
