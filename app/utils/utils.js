...
animateVh = (dir, elem, setInitialState = false) => {
  ...
  if (dir === "open") {
    if (parseInt(elem.style.height, 10) < window.innerHeight * 1.2) {
      const incr = String(
        parseInt(elem.style.height, 10) + window.innerHeight / 20
      );
      elem.style.height =
        incr > window.innerHeight * 1.2
          ? window.innerHeight * 1.2 + 1 + "px"
          : incr + "px";
    ...
    if (parseInt(elem.style.height, 10) > 0) {
      const subst = String(
        parseInt(elem.style.height, 10) - window.innerHeight / 13
      );
